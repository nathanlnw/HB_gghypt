		...
		case KeyValueOk:
			if((password_SetFlag>=1)&&(password_SetFlag<=6))
				...
				if(strncmp((char *)password_Code,ENTER_CODE,6)==0)    
					{
					if(MENU_set_InsertMode_flag==1)
						{
						password_SetFlag=0;
						MENU_set_InsertMode_flag=2;
						lcd_fill(0);
						lcd_text12(36,3,"密码正确",8,LCD_MODE_SET);
						lcd_text12(6,19,"按确认设置接入模式",18,LCD_MODE_SET);
						lcd_update_all();
			...
			else if(MENU_set_InsertMode_flag==2)
				{
				MENU_set_InsertMode_flag=0;
				pMenuItem=&Menu_5_9_mode;
				pMenuItem->show();
				}
