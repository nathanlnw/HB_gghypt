...
char	Dialinit_TDT_DNSR3[50]		=	"AT%DNSR=\"";	  //  域名
char	Dialinit_TDT_DNSR4[50]		=	"AT%DNSR=\"";	  //  域名
...
u8    Dial_jump_State=0;  // 拨号过程中  状态跳转标志， DialInit5  DialInit6 到 DialInit7 的跳转标记
                         //                            dns1     dns2     dns3   dns4
						 //   Dial_jump_State=7          8          9       10   
u8    Dnsr_state=0;  //  DNSR 状态 0: 没有启用域名分析   1: dnsr1 成功解析 2: dnsr2成功解析
                     //                         3: dnsr_tdt_dnsr3 成功解析 4: dnsr4_tdt_dnsr4成功解析
...
void  DataLink_DNSR2_Set(u8* Dns_str,u8 DebugOUT)
{
 ...
 void  DataLink_DNSR_Init(u8 type,u8* Dns_str,u8 DebugOUT)
 {
    switch(type)
    {
       case 1:   // dns1
	   ...
	   case 2://  dns2
	           ...
	           break;
	   case 3:  // dns3   tdt-1
                   	memset(Dialinit_TDT_DNSR3+9,0,sizeof(Dialinit_TDT_DNSR3)-9); 
					memcpy(Dialinit_TDT_DNSR3+9,Dns_str,strlen((char const*)Dns_str)); 
					strcat( Dialinit_TDT_DNSR3,"\"\r\n" ); 

					if(DebugOUT)
					{
					rt_kprintf("\r\n	 TDT 1 域名 :  "); 
					rt_kprintf(Dialinit_TDT_DNSR3); 
					}
		       break;
	   case 4:  //  dns4  tdt-2
                 	memset(Dialinit_TDT_DNSR4+9,0,sizeof(Dialinit_TDT_DNSR4)-9); 
					memcpy(Dialinit_TDT_DNSR4+9,Dns_str,strlen((char const*)Dns_str)); 
					strcat( Dialinit_TDT_DNSR4,"\"\r\n" ); 

					if(DebugOUT)
					{
					rt_kprintf("\r\n	 TDT 2 域名 :  "); 
					rt_kprintf(Dialinit_TDT_DNSR4);  
					}
		       break;   
...
{  
  ...
  DataLink_DNSR_Init(3,TDT_MainDnsStr,0);
  DataLink_DNSR_Init(4,TDT_AuxDnsStr,0); 
         ...
         if(Dnsr_state)
            {   
              ...
              switch(Dnsr_state)
              	{
                  case 1:  // dns1
                         Dial_Stage(Dial_DNSR2); // 尝试第2个域名   
					     break;
	              case 2:  // dns2
                         Dial_Stage(Dial_TDT_DNS3); // 尝试第3个域名 
			...
			DataLink_AuxSocket_set(RemoteIP_aux, RemotePort_tdt,0);	 
       ...
       if(DataDial.Dial_ON)
       {
            ...
            case Dial_DNSR2:      rt_hw_gsm_output(Dialinit_DNSR2);  // Aux DNSR
					...
					for(i=0;i<len;i++)										
					         rt_kprintf("%c",Dialinit_DNSR2[i]);       
                                   Dial_jump_State=8; 
					Dnsr_state=2;//表示DNSR 状态下拨号 				   
				       break;	
			//----------------  add  for  tdt  version  ----------------------------------
            case Dial_TDT_DNS3:    rt_hw_gsm_output(Dialinit_TDT_DNSR3);  // tdt DNSR3
                                   DataDial.Dial_step_RetryTimer=Dial_max_Timeout; 
					DataDial.Dial_step_Retry++;
					rt_kprintf("\r\n   ----- tdt  DNSR3 ----\r\n");
					 len=strlen((const char*)Dialinit_TDT_DNSR3);
					for(i=0;i<len;i++)										
					         rt_kprintf("%c",Dialinit_TDT_DNSR3[i]);       
                                   Dial_jump_State=9; 
					Dnsr_state=3;//表示DNSR 状态下拨号 				   
				       break;

			case Dial_TDT_DNS4:	rt_hw_gsm_output(Dialinit_TDT_DNSR4);  // tdt DNSR4
										  DataDial.Dial_step_RetryTimer=Dial_max_Timeout; 
						   DataDial.Dial_step_Retry++;
						   rt_kprintf("\r\n   -----tdt  DNSR4 ----\r\n");
							len=strlen((const char*)Dialinit_TDT_DNSR4);
						   for(i=0;i<len;i++)									   
									rt_kprintf("%c",Dialinit_TDT_DNSR4[i]); 	  
						   Dial_jump_State=10; 
						   Dnsr_state=4;//表示DNSR 状态下拨号		 		  
					   break;
		    ...
		    case Dial_DialInit7:    rt_hw_gsm_output(DialInit8);  // Query  IP 
		     ...
		     case Dial_MainLnk:   // rt_hw_gsm_output(DialStr_Link1); 
		             if( Dnsr_state==0)//表示DNSR 状态下失败拨号连接
		                       	{
		                       	   rt_kprintf("\r\n Dial  orginal   Mainlink\r\n");  
                                   DataLink_MainSocket_set(RemoteIP_main,RemotePort_tdt,1);
		                       	}
					 rt_kprintf("\r\n -----------连接------------\r\n");  
                     DataDial.Dial_step_RetryTimer=300; //3000
					 DataDial.Dial_step_Retry++;
...
//----------------------  Debug -------------------------		
  ...
  /*     if(strncmp((char*)GSM_rx, "%IPDATA:2",9)==0)   // ISP 过来      
				...
				switch(Dial_jump_State)
					{
						...
						case 9:  
						case 10:
							    DataLink_MainSocket_set(RemoteIP_Dnsr,RemotePort_tdt,1);
								Dial_jump_State=0; 
								break;
						default:
							     return;
	...
	{ 
			...
			// M66  联通卡+COPS: 0,0,"CHINA UNICOM",0
			// M66  移动卡+COPS: 0,0,"CHINA MOBILE",0
			//注销的卡	  +COPS: 0
	...
	if((strncmp((char*)GSM_rx,"ERROR: 13",9)==0)||(strncmp((char*)GSM_rx,"ERROR:13",8)==0))//ERROR:13
	{
		       ...
		       switch(Dial_jump_State)
		       {
		          case  7 :   // dns1
							{  
							   rt_hw_gsm_output("AT%IPCLOSE=1\r\n");   
							   rt_thread_delay(10);   
					           Dial_Stage(Dial_DNSR2);  // 后边跳跃到ready to  connect aux DNS 
							   rt_kprintf("\r\n   DNSR1  解析 成功-->连接失败\r\n");   	
							   Dnsr_state=0;//表示DNSR 状态下拨号 	 
							   break;	   
							}
		...
		case Dial_DialInit6	:	if (ok)  
			                  /*
			                   ...
			                   if(Vechicle_Info.Link_Frist_Mode==1)
								...
								if((Vechicle_Info.Vech_MODE_Mark==2)||(Vechicle_Info.Vech_MODE_Mark))  //  货运模式	
                                {
                                    Dial_Stage(Dial_TDT_DNS3);  // 普通模式 连接 天地通域名  
                                    rt_kprintf("\r\n  二客一危模式连接\r\n"); 
								 }
