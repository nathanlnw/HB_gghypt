...
u16		RemotePort_main=7008;// 8201;//天津9131;   河北天地通 8201             //test tianjin     
u16     RemotePort_tdt=8201;//  天地通 中心 端口
...
u8           TDT_MainDnsStr[50]="up.gps960.net"; ;  // 域名  天地通up.gps960.com //jt1.gghypt.net 
u8           TDT_AuxDnsStr[50]="up.Soo8.com";     //"www.sina.com";//jt2.gghypt.net
...
u8  SysConfig_init(void)
{
       ...
       //  1. Stuff
		...
		memcpy(SysConf_struct.DNSR_Aux,(u8*)DomainNameStr_aux,strlen((const char*)DomainNameStr_aux));   
                            //   域名tdt  1
		memset((u8*)SysConf_struct.DNSR_TDT,0 ,sizeof(SysConf_struct.DNSR_TDT));
		memcpy(SysConf_struct.DNSR_TDT,(u8*)TDT_MainDnsStr,strlen((const char*)TDT_MainDnsStr));   
                            //   域名tdt  2
		memset((u8*)SysConf_struct.DNSR_TDT_Aux,0 ,sizeof(SysConf_struct.DNSR_TDT_Aux));
		memcpy(SysConf_struct.DNSR_TDT_Aux,(u8*)TDT_AuxDnsStr,strlen((const char*)TDT_AuxDnsStr));    


	        //   主 IP   +  端口
	        memcpy(SysConf_struct.IP_Main,(u8*)RemoteIP_main,4); 
	        SysConf_struct.Port_main=RemotePort_main;
			SysConf_struct.Port_tdt=RemotePort_tdt; //天地通端口
	       //   备用 IP   +  端口
...
void SysConfig_Read(void)
{
		...
		memcpy((u8*)DomainNameStr_aux,SysConf_struct.DNSR_Aux,strlen((const char*)SysConf_struct.DNSR_Aux)); 
	                        //   tdt  dnsr3
		memset((u8*)TDT_MainDnsStr,0 ,sizeof(TDT_MainDnsStr));
		memcpy((u8*)TDT_MainDnsStr,SysConf_struct.DNSR_TDT,strlen((const char*)SysConf_struct.DNSR_TDT)); 
								//	 tdt  dnsr4
		memset((u8*)TDT_AuxDnsStr,0 ,sizeof(TDT_AuxDnsStr));
		memcpy((u8*)TDT_AuxDnsStr,SysConf_struct.DNSR_TDT_Aux,strlen((const char*)SysConf_struct.DNSR_TDT_Aux)); 
	  ...
	  rt_kprintf("\r\n继电器闭合");
        ...
        // APN 设置
         ...
         DataLink_APN_Set(APN_String,1); 
		  ...
		  //-- dns3
           memset(reg_str,0,sizeof(reg_str));
          memcpy(reg_str,TDT_MainDnsStr,strlen((char const*)TDT_MainDnsStr));
          rt_kprintf("\r\n		tdt dns3 域名设置 :	 %s\r\n",reg_str);
		  //--- dns4
		  memset(reg_str,0,sizeof(reg_str));
          memcpy(reg_str,TDT_AuxDnsStr,strlen((char const*)TDT_AuxDnsStr)); 
          rt_kprintf("\r\n		tdt dns4  域名设置 :	 %s\r\n",reg_str);    
		  
					  
         // 数据中心IP 地址(4Bytes)  UDP端口号码(2Bytes) TCP端口号码(2Bytes)
         rt_kprintf("\r\n		  主IP: %d.%d.%d.%d : %d     天地通port:%d\r\n",RemoteIP_main[0],RemoteIP_main[1],RemoteIP_main[2],RemoteIP_main[3],RemotePort_main,RemotePort_tdt);   
